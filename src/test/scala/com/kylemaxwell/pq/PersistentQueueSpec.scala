...
class PersistentQueueSpec extends Specification with TestHelper {
  ...
  // convenience
  def makeQueue(name: String) = {
    new PersistentQueue(folderName, name)
  }
  ...
  "PersistentQueue" should {
    ...
    "honor max_age" in {
      withCurrentTimeFrozen { time => 
        withTempFolder {
          val q = makeQueue("weather_updates")
          q.maxAge.set(Some(3))
          q.setup
    ...
    "allow max_journal_size and max_memory_size to be overridden per queue" in {
      withTempFolder {
        val q1 = makeQueue("test1")
        q1.maxMemorySize.set(Some(123))
        q1.maxJournalSize() mustEqual PersistentQueue.maxJournalSize
        q1.maxMemorySize() mustEqual 123
        val q2 = makeQueue("test2")
        q2.maxJournalSize.set(Some(123))
    ...
    "recover a journal with open transactions" in {
      withTempFolder {
        val q = new PersistentQueue(folderName, "things")
        q.setup
        q.add("one".getBytes)
        q.add("two".getBytes)
        ...
        val q2 = new PersistentQueue(folderName, "things")
        q2.setup
    ...
    "recreate the journal file when it gets too big" in {
      withTempFolder {
        val q = makeQueue("things")
        q.maxJournalSize.set(Some(1024))
        q.maxJournalOverflow.set(Some(3))
        q.setup
  ...
  "PersistentQueue with no journal" should {
    ...
    "lose all data after being destroyed" in {
      withTempFolder {
        ...
        val q2 = makeQueue("mem")
        q2.keepJournal.set(Some(false))
        q2.setup
  ...
  "PersistentQueue with item/size limit" should {
    ...
    "drop older items when discard_old_when_full is set" in {
      withTempFolder {
        val q = makeQueue("weather_updates")
        q.maxItems.set(Some(3))
        q.discardOldWhenFull.set(Some(true))
        q.setup
        q.add("sunny".getBytes) mustEqual true
        q.add("rainy".getBytes) mustEqual true
  ...
  "PersistentQueue with item expiry" should {
    ...
    "expire items into a queue" in {
      withCurrentTimeFrozen { time =>
        withTempFolder {
          val r = makeQueue("rappers")
          r.keepJournal.set(Some(false))
          val q = makeQueue("wu_tang")
          q.keepJournal.set(Some(false))
