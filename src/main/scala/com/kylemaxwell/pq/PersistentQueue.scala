...
class PersistentQueue(persistencePath: String, val name: String) {
  ...
  def replayJournal(): Unit = {
    ...
    log.info("Finished transaction journal for '"+name+"' ("+queueLength+" items, "+journal.size+" bytes)")
    journal.open
...
object PersistentQueue {
  ...
  def apply(path: String, name: String, maxItems: Int, maxSize: Long, maxItemSize: Long,
            ...
            maxExpireSweep: Int) = {
    val queue = new PersistentQueue(path, name)
    ...
    log.info("Configuring queue "+name+": journal="+keepJournal+", max_items="+maxItems+", max_size="+maxSize+", max_age="+maxAge+", max_journal_size="+maxJournalSize+", max_memory_size="+maxMemorySize+", max_journal_overflow="+maxJournalOverflow+", max_journal_size_absolute=maxJournalSizeAbsolute, discard_old_when_full="+discardOldWhenFull+", sync_journal="+syncJournal)
    queue
  }
}
