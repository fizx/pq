          ...
          readJournalEntry(in) match {
            ...
            case (x, itemsize) =>
              ...
              if (size / TEN_MB > lastUpdate) {
                ...
                log.info("Continuing to read '"+name+"' journal; "+(lastUpdate*10)+" MB so far...")
              }
          }
        } while (!done)
      } catch {
        case e: BrokenItemException =>
          log.error("Exception replaying journal for '"+name+"'", e)
          log.error("DATA MAY HAVE BEEN LOST! Truncated entry will be deleted.")
          truncateJournal(e.lastValidPosition)
      }
    } catch {
      case e: FileNotFoundException =>
        log.info("No transaction journal for '"+name+"'; starting with empty queue.")
      case e: IOException =>
        log.error("Exception replaying journal for '"+name+"'", e)
